...
namespace ServerGridEditor
{
	 public partial class MainForm : Form
	 {
		  ...
		  Island> islands = new Dictionary<string,
		  Island>();
		  ...
		  public void DrawMapToGraphics(ref Graphics g, bool cull = false, bool ignoreTranslation = false, bool forExport = false)
		  {
				...
				DrawMap(
				this, islands, g, showLinesCheckbox.Checked, showServerInfoCheckbox.Checked, showGridNameChckBox.Checked,
				showIslandsChckBox.Checked, showDiscoZoneInfoCheckbox.Checked, culling, alphaBackground,
				tiledBackgroundCheckbox.Checked ? tile : null, tiledBackgroundCheckbox.Checked ? tileBrush : null, tileScaleBox.Value,
				!ignoreTranslation ? mapHScrollBar.Value : 0, !ignoreTranslation ? mapVScrollBar.Value : 0, forExport,
				showPathingGridCheckbox.Checked);
		  }
		  ...
		  MainForm mainForm, IDictionary<string, Island> islands, Graphics g, bool showLines, bool showServerInfo,
		  bool showGridName, bool showIslands, bool showDiscoZoneInfo, RectangleF? culling,
		  Color? alphaBackground, Image tile, TextureBrush tileBrush, decimal tileScale, int translateH, int translateV,
		  bool forExport, bool bShowPathingGrid)
		  {
				...
				if (showGridName)
				{
					 ...
					 foreach (Server s in currentProject.servers)
					 {
						  ...
						  string easyName = string.Format("{0}{1}", (char)('A' + s.gridX), s.gridY + 1);
				...
				if (showServerInfo)
				{
					 ...
					 new float[] {
						...
						1
									 ...
									 g.DrawImage(lockImg, new Rectangle((int)(serverCenter.X + cellSize / 2 - lockImgSize), (int)(serverCenter.Y - cellSize / 2), (int)lockImgSize, (int)lockImgSize), 0, 0, lockImg.Width, lockImg.Height, GraphicsUnit.Pixel, ia);
					 ...
					 new float[] {
						...
						1
									 ...
									 g.DrawImage(lockImg, new Rectangle((int)(serverCenter.X + cellSize / 2 - lockImgSize), (int)(serverCenter.Y - cellSize / 2 + locksShift), (int)lockImgSize, (int)lockImgSize), 0, 0, lockImg.Width, lockImg.Height, GraphicsUnit.Pixel, ia);
									 locksShift += lockImgSize;
								}
								if (s.pathsLocked)
								{
		  ...
		  private void mapPanel_MouseMove(object sender, MouseEventArgs e)
		  {
				...
				else if (CurrentHeldMoveableObject != null)
				{
					 if (CurrentHeldMoveableObject is DiscoveryZoneData && bResizingDiscoZone)
					 {
						  ...
						  this, new PointF(CurrentHeldDiscoZoneInstance.startWorldX - CurrentHeldDiscoZoneInstance.sizeX / 2, CurrentHeldDiscoZoneInstance.startWorldY - CurrentHeldDiscoZoneInstance.sizeY / 2));
					 }
					 else
					 {
						  CurrentHeldMoveableObject.SetWorldLocation(
						  this, new PointF(CurrentHeldMoveableObject.worldX + moveDelta.X / currentProject.coordsScaling, CurrentHeldMoveableObject.worldY + moveDelta.Y / currentProject.coordsScaling));
					 }
				...
				else if (selectedMovableObjects.Count > 0)
				{
					 if (selectionBoxHeld)
					 {
						  ...
						  for (int i = 0; i < selectedMovableObjects.Count; i++)
						  {
								...
								this, new PointF(selectedMovableObjects[i].worldX + moveDelta.X / currentProject.coordsScaling, selectedMovableObjects[i].worldY + moveDelta.Y / currentProject.coordsScaling));
						  }
		  ...
		  public void SaveIslands(string islandRemovedFromMod = null)
		  {
				...
				Dictionary<string,
				Dictionary<string,
				Island>> modsDict = new Dictionary<string,
				Dictionary<string,
				Island>>();

				List<Island> allIslands = islands.Values.ToList();
				Dictionary<string,
				Island> originalIslands = new Dictionary<string,
				Island>();
				...
				foreach (Island island in allIslands)
				{
					 if (!string.IsNullOrWhiteSpace(island.modDir))
					 {
						  ...
						  Dictionary<string,
						  Island> modIslandsDict = null;
						  if (!modsDict.TryGetValue(island.modDir, out modIslandsDict))
						  {
								modIslandsDict = new Dictionary<string,
								Island>();
		  ...
		  public void LoadIslands()
		  {
				if (File.Exists(islandsSaveFile))
				{
					 islands = JsonConvert.DeserializeObject<Dictionary<string,
					 Island>>(File.ReadAllText(islandsSaveFile));
					 ...
					 foreach (string islandModDir in modDirs)
					 {
						  ...
						  {
								Dictionary<string,
								Island> ModIslands = JsonConvert.DeserializeObject<Dictionary<string,
								Island>>(File.ReadAllText(dataFile));
								if (ModIslands != null)
								{
									 foreach (KeyValuePair<string, Island> kvp in ModIslands)
									 {
										  ...
										  {
												MessageBox.Show(string.Format("File {0} contains duplicate island named {1}, this island will not be imported", Path.GetFileName(dataFile), kvp.Key), "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
												continue;
										  }
		  ...
		  void LoadProject()
		  {
				if (currentProject != null)
				{
					 var confirmResult = MessageBox.Show("If you click OK, any unsaved changes to your current project will be LOST!", "Warning", MessageBoxButtons.OKCancel);
		  ...
		  private void slippyMapToolStripMenuItem_Click(object sender, EventArgs e)
		  {
				...
				using (var exportMapForm = new ExportSlippyMap())
				{
					 ...
					 MessageBox.Show("Slippy Map exported.", "Slippy Map Exported", MessageBoxButtons.OK, MessageBoxIcon.Information);
				}
		  }
		  ...
		  private void cullInvalidPathsToolStripMenuItem_Click(object sender, EventArgs e)
		  {
				...
				currentProject.shipPaths.RemoveAll((path) =>
				{
					 ...
					 {
						  if (node.worldX < 0 || node.worldX > maxDimensions.X || node.worldY < 0 || node.worldY > maxDimensions.Y)
						  {
								invalidCount++;
								return true;
						  }
